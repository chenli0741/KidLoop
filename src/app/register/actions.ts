...
export async function register(_: FormState, form: FormData): Promise<FormState> {
  ...
  if (!["ADMIN","DRIVER","PARENT"].includes(role) || !name || name.length>100 || email.length>254 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || password.length<6 || password.length>128 || password!==form.get("confirmPassword")) {
    return {ok:false,message:text(locale,"请选择注册身份，填写姓名、有效邮箱及两次一致的密码（6–128 字符）。","Choose your role and enter your name, a valid email and matching passwords (6–128 characters).")};
  }
  ...
  try {
    await identityQuery("insert into login_accounts(email,name,password_hash,registration_role) values($1,$2,$3,$4)",[email,name,await hashPassword(password),role]);
