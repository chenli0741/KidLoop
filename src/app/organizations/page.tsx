...
export default async function OrganizationsPage({searchParams}:{searchParams:Promise<{unavailable?:string}>}) {
  ...
  return <div className={user?'page-container institution-page':'login-page'}><section className={user?'':'login-card institution-card'}>
    ...
    <p>{user?text(l,`当前机构：${user.tenantName}。如需进入其他机构，请退出后重新登录。`,`Current institution: ${user.tenantName}. Sign out and sign in to enter another institution.`):text(l,"选择本次登录的接送机构，或创建、申请加入机构。","Select an institution for this login, or create or request to join one.")}</p>
