...
export default async function AccountsPage() {
  ...
  const [accounts, drivers, students] = await Promise.all([
    ...
    `), getDrivers(), query<{id:string;name:string;schoolName:string}>(`select st.id,st.name,coalesce(sc.short_name,sc.name) as "schoolName" from students st join schools sc on sc.id = st.school_id where st.active or exists(select 1 from user_students us where us.student_id=st.id) order by st.name`).then(result=>result.rows),
  ]);
  const labels = { ADMIN: text(locale, "管理员", "Admin"), DRIVER: text(locale, "司机", "Driver"), PARENT: text(locale, "家长", "Parent") };
  return <div className="page-container"><PageHeader eyebrow="Kid Loop Rides" title={text(locale, "登录账号", "Login accounts")} description={text(locale, "管理三个角色的账号，明确绑定司机和孩子。", "Manage three roles and explicitly link drivers and children.")} />
    ...
    <div className="split-layout desktop-only"><section className="account-list">{accounts.rows.map((account) => <article className="account-card" key={account.id}>
      ...
      <details className="account-details"><summary><KeyRound size={15} />{text(locale, "重置密码", "Reset password")}</summary><ActionForm action={resetAccountPassword} submitLabel={text(locale, "重置密码", "Reset password")}><input type="hidden" name="userId" value={account.id} /><label className="full"><span>{text(locale, "新密码（6–128 字符）", "New password (6–128 characters)")}</span><input type="password" name="password" minLength={6} maxLength={128} autoComplete="new-password" required /></label></ActionForm></details>
