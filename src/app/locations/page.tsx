...
export default async function LocationsPage() {
  ...
  return (
    <div className="page-container">
      <PageHeader eyebrow={text(locale, "接送地点", "Pickup & dropoff")} title={text(locale, "地点", "Locations")} description={text(locale, "保存司机在每个接送点需要的准确说明。", "Store the exact instructions drivers need at every stop.")} />
      ...
      <div className="split-layout">
        <section className="content-section">
          <div className="section-heading"><div><span className="eyebrow">{text(locale, "送达", "Dropoff")}</span><h2>{text(locale, "课外班", "After-school programs")}</h2></div><span className="section-count">{programs.length}</span></div>
          ...
          ))}</div> : <EmptyState title={text(locale, "暂无课外班", "No programs")} body={text(locale, "添加第一个送达地点。", "Add the first dropoff location.")} />}
        </section>
        <FormPanel heading={<div className="panel-heading"><MapPin size={19} /><div><h2>{text(locale, "添加课外班", "Add program")}</h2><p>{text(locale, "司机送达信息", "Driver dropoff details")}</p></div></div>}>
          <ActionForm action={createProgram} submitLabel={text(locale, "添加课外班", "Add program")}>
            <label><span>{text(locale, "课外班名称", "Program name")}</span><input name="name" required /></label>
            <label><span>{text(locale, "地址", "Address")}</span><input name="address" required /></label>
            <label className="full"><span>{text(locale, "送达信息", "Dropoff information")}</span><textarea name="dropoffInfo" rows={3} required /></label>
            <label className="full"><span>{text(locale, "课外班要求", "Program requirements")}</span><textarea name="requirements" rows={3} required /></label>
          </ActionForm>
        </FormPanel>
      </div>
    </div>
