...
async function Calendar({dates,selected,today,view,locale}:{dates:string[];selected:string;today:string;view:string;locale:Locale}){
 ...
 if(view==='week') return <><div className="calendar-weekdays review-weekdays" aria-hidden="true">{(locale==='zh'?['一','二','三','四','五']:['M','T','W','T','F']).map((label,index)=><span key={index}>{label}</span>)}</div><div className="review-calendar review-week-calendar">{dates.map(date=>{
  ...
  const label=date<today?text(locale,'过去','Past'):!summary||!summary.checked?text(locale,'待核对','Review'):summary.issueCount?text(locale,`异常 ${summary.issueCount}`,`${summary.issueCount} issues`):summary.holiday?text(locale,'无接送','No pickups'):summary.hasTrips?text(locale,'有行程','Rides'):text(locale,'无行程','No rides');
 ...
 const cell=(date:string)=>{const summary=summaries.find(s=>s.date===date);const label=date<today?text(locale,'过去','Past'):!summary||!summary.checked?text(locale,'待核对','Review'):summary.issueCount?text(locale,`异常 ${summary.issueCount}`,`${summary.issueCount} issues`):summary.holiday?text(locale,'无接送','No pickups'):summary.hasTrips?text(locale,'有行程','Rides'):text(locale,'无行程','No rides');return <Link prefetch={false} key={date} href={`?date=${date}&view=${view}`} className={`review-date is-${scheduleDatePeriod(date,today)} ${date===selected?'is-selected':''}`} aria-label={`${date} ${label}`} aria-current={date===selected?'date':undefined}><strong>{date===today?text(locale,'今天','Today'):Number(date.slice(8))}</strong><span className={summary?.issueCount?'review-issue-label':''}>{label}</span></Link>};
