...
export default async function ProfilePage() {
  ...
  return <div className="page-container"><PageHeader eyebrow={text(locale, "我的账号", "My account")} title={text(locale, "个人资料", "Personal information")} description={text(locale, "管理你的联系信息、登录邮箱和密码。", "Manage your contact information, login email, and password.")} />
    <div className="profile-grid"><section className="profile-panel"><h2>{text(locale, "账号信息", "Account information")}</h2><p className="form-hint">{text(locale, "修改登录邮箱会同步到所有机构，需验证当前密码。", "Changing your login email applies across all institutions and requires your current password.")}</p>
      <SettingsForm action={updateProfile} submitLabel={text(locale, "保存个人资料", "Save profile")}>
        <PhotoUpload current={user.photoUrl ?? ""} purpose="avatar" />
        ...
        <label className="full"><span>{text(locale,"当前密码（修改邮箱时填写）","Current password (to change email)")}</span><input name="currentPassword" type="password" autoComplete="current-password" maxLength={128}/></label>
      </SettingsForm>
