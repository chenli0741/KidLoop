...
export default async function ResourcesPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const tab = value === "schools" || value === "programs" || value === "travel" ? value : "fleet";
  const tabs = [{ id: "fleet", label: text(locale, "车队", "Fleet"), icon: BusFront }, { id: "schools", label: text(locale, "学校", "Schools"), icon: School }, { id: "programs", label: text(locale, "培训学校", "Programs"), icon: MapPin }, { id: "travel", label: text(locale, "地点时间", "Travel times"), icon: MapPin }];
  return <div className="page-container"><PageHeader eyebrow={text(locale, "基础资料", "Resources")} title={text(locale, "车队与学校", "Fleet & schools")} description={text(locale, "统一维护车辆、司机、学校和培训学校的接送资料。", "Manage vehicles, drivers, schools, and after-school pickup and dropoff details.")} />
    ...
    {tab === "fleet" ? <FleetResources /> : tab === "travel" ? <TravelTimeResources /> : <LocationResources section={tab} />}
  </div>;
}
