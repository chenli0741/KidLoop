...
export async function createAccount(_: FormState, form: FormData): Promise<FormState> {
  ...
  try {
    ...
    return { ok: false, message: text(locale, "创建失败。邮箱和司机不能重复，密码需 6–128 个字符，家长需绑定孩子，司机需绑定司机资料。", "Could not create account. Email and driver must be unique, password must have 6–128 characters, and the required child or driver link must be selected.") };
  }
}
...
export async function resetAccountPassword(_: FormState, form: FormData): Promise<FormState> {
  ...
  try {
    ...
    return { ok: false, message: text(locale, "无法重置密码，请输入 6–128 个字符。", "Could not reset password. Use 6–128 characters.") };
  }
}
