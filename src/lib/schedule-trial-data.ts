...
export async function readTrialInput(c:SqlReader,start:string,end:string):Promise<TrialInput>{
 ...
 const [routes,roster,terms,exceptions,drivers,vehicles,absences,existing,travelTimes,driverRuns,shifts,driverUnavailability]=await Promise.all([
  ...
  ,c.query(`select id,driver_id as "driverId",starts_on::text as "startsOn",ends_on::text as "endsOn",weekdays,to_char(unavailable_from,'HH24:MI') as "unavailableFrom",to_char(unavailable_to,'HH24:MI') as "unavailableTo",reason from driver_unavailability where starts_on<=$2 and ends_on>=$1 order by starts_on,id`,[start,end])
 ]);
 return {routes,...roster,driverRuns,driverUnavailability:driverUnavailability.rows,terms:terms.rows,exceptions:exceptions.rows,drivers:drivers.rows,vehicles:vehicles.rows,absences:absences.rows,travelTimes:travelTimes.rows,existing:[...existing.rows,...shifts.rows]};
}
