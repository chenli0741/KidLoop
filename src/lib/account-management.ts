...
export async function saveAccount(c: PoolClient, actor: AuthUser, form: FormData) {
  ...
  if ((role !== "DRIVER" && !name) || name.length > 100 || phone.length > 80 || email.length > 254 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || !["ADMIN", "DRIVER", "PARENT"].includes(role) || (role === "DRIVER" && !driver)) throw new AccountEditError("invalid");
  const current = (await c.query("select id,email,role,driver_id,updated_at::text as version from app_users where id=$1 for update", [id])).rows[0];
  if (current && email !== current.email) throw new AccountEditError("emailImmutable");
  ...
  await c.query("update app_users set name=$2,phone=$3,role=$4,driver_id=$5,updated_at=clock_timestamp() where id=$1", [id,driver ? null : name,driver ? null : phone,role,driver]);
