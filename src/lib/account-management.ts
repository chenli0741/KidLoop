...
export async function saveAccount(c: PoolClient, actor: AuthUser, form: FormData) {
  ...
  await c.query("update app_users set name=$2,email=$3,phone=$4,role=$5,driver_id=$6,updated_at=clock_timestamp() where id=$1", [id,driver ? null : name,email,driver ? null : phone,role,driver]);
