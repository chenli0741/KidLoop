...
export type FixedRoute = {sharing?: {id:string;sourceRouteId:string;partnerRouteId:string;schoolId:string};notes?:string;id:string; name:string; routeType:'RECURRING'|'TEMPORARY'; startsOn:string; endsOn:string; weekdays:number[]; driverId:string|null; vehicleId:string|null; enabled:boolean; updatedAt:string; stops:RouteStop[]; students:RouteStudent[]};
