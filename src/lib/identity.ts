...
export const getSessionHash = cache(async () => {
...
export const getIdentity = cache(async () => {
  ...
  if (!hash) return null;
  const row = (await identityQuery<{id:string;email:string;name:string;tenantId:string|null;contextKey:string;registrationRole:UserRole}>(`
    select a.id,a.email,a.name,a.registration_role as "registrationRole",s.selected_tenant_id as "tenantId",s.context_key as "contextKey"
