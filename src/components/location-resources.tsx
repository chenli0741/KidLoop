...
import { createProgram, createSchool, updateSchool } from "@/app/actions";
      ...
      {section === "schools" && (<div className="split-layout">
        <section className="content-section">
          ...
          {schools.length ? <div className="location-list">{schools.map((school) => (
            <article className="location-card" key={school.id}>
              <div className="location-title"><span className="record-icon"><School size={20} /></span><div><h3>{school.name}</h3>{school.fullName !== school.name && <p>{school.fullName}</p>}<p><MapPin size={14} /> {school.address || text(locale, "地址待补充", "Address pending")}</p></div></div>
              <details><summary>{text(locale, "编辑学校名称", "Edit school names")}</summary>
                <ActionForm key={school.updatedAt} action={updateSchool} submitLabel={text(locale, "保存名称", "Save names")}>
                  <input type="hidden" name="id" value={school.id} />
                  <input type="hidden" name="updatedAt" value={school.updatedAt} />
                  <label><span>{text(locale, "学校全称", "Full school name")}</span><input name="name" required maxLength={160} defaultValue={school.fullName} /></label>
                  <label><span>{text(locale, "简称（选填）", "Short name (optional)")}</span><input name="shortName" maxLength={80} defaultValue={school.shortName ?? ""} /></label>
                  <p className="form-hint full">{text(locale, "日常页面优先显示简称；未填写时显示全称。", "Everyday screens use the short name, or the full name if left blank.")}</p>
                </ActionForm>
              </details>
              <div className="location-details"><Link href={`/schedule?school=${school.id}`}>{text(locale, "学校日历与接送规则", "School calendar & pickup rules")} →</Link><p>{school.pickupInstructions}</p><div className="route-links">{school.address && <LocationMap name={school.name} address={school.address} />}<LocationMap name={school.name} url={school.pickupMapUrl} /></div></div>
            </article>
        ...
        <FormPanel heading={<div className="panel-heading"><School size={19} /><div><h2>{text(locale, "添加学校", "Add school")}</h2><p>{text(locale, "司机接学生信息", "Driver pickup details")}</p></div></div>}>
          <ActionForm action={createSchool} submitLabel={text(locale, "添加学校", "Add school")}>
            <label><span>{text(locale, "学校全称", "Full school name")}</span><input name="name" required maxLength={160} />
            </label><label><span>{text(locale, "简称（选填）", "Short name (optional)")}</span><input name="shortName" maxLength={80} /></label>
