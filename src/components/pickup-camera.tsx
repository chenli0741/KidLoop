...
function CameraDialog({trip,locale,onUpdated,close,initialImage}:{trip:Trip;locale:Locale;onUpdated:(u:TripExecution)=>void;close:()=>void;initialImage:string}){
 ...
 async function analyze(image:string){
  ...
  try{const {recognizePickup}=await import('@/lib/pickup-camera/local-recognition');const result=await recognizePickup(image,trip.riders,controller.signal,(done,total)=>setProgress(text(locale,`正在对照学生照片 ${done}/${total}`,`Matching student photos ${done}/${total}`)));if(controller.signal.aborted)return;
   setMatches(result.matches);setSelected(result.matches.map(m=>m.assignmentId??''));setProgress(result.matches.length?text(locale,`检测到 ${result.matches.length} 张人脸；正在检查安全带。`,`Detected ${result.matches.length} faces. Checking seat belts.`):text(locale,'未检测到人脸，请重拍。','No faces detected. Retake the photo.'));
   ...
   if(result.matches.length){try{const response=await fetch('/api/pickup-camera/seatbelts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tripId:trip.id,image,faces:result.matches.map((m,index)=>({index,...m.face}))}),signal:AbortSignal.any([controller.signal,AbortSignal.timeout(15000)])});if(!response.ok)throw new Error();const data=await response.json();if(!controller.signal.aborted)setBelts(Object.fromEntries(data.results.map((r:{index:number;status:string})=>[r.index,r.status])));}catch{if(!controller.signal.aborted)setBelts(Object.fromEntries(result.matches.map((_,i)=>[i,'UNAVAILABLE'])));}}
 ...
 <div className={s.frame}>{shot?<><img src={shot} alt={text(locale,'本次车内画面','Current cabin snapshot')}/>{matches.map((m,i)=><div key={i} className={s.box} data-unknown={!m.assignmentId} style={{left:`${Math.max(0,m.face.x)*100}%`,top:`${Math.max(0,m.face.y)*100}%`,width:`${m.face.width*100}%`,height:`${m.face.height*100}%`}}><span>{i+1} · {trip.riders.find(r=>r.id===m.assignmentId)?.name??text(locale,'未识别','Unknown')}{m.assignmentId&&!eligible.some(r=>r.id===m.assignmentId)?text(locale,'（非待接）',' (not pending)'):''}{belts[i]==='CHECK'?' ⚠':''}</span></div>)}</>:<video ref={video} muted playsInline autoPlay/>}</div>
 <p role="status">{progress}</p>{error&&<p className={s.error} role="alert">{error}</p>}
 {matches.map((match,i)=>{const rider=trip.riders.find(r=>r.id===match.assignmentId),canPick=eligible.some(r=>r.id===match.assignmentId);return <div key={i} className={s.row}><span>{i+1}</span>{rider?<label><input type="checkbox" disabled={saving||!canPick} checked={canPick&&selected[i]===rider.id} onChange={e=>setSelected(old=>old.map((id,j)=>j===i?(e.target.checked?rider.id:''):id))}/>{rider.name}{!canPick&&<span> · {text(locale,'无需重复确认','No pickup update needed')}</span>}</label>:<span>{text(locale,'未识别','Unknown')}</span>}<small>{beltLabel(belts[i])}</small></div>;})}
 {shot&&!busy&&<p className={s.notice}>{text(locale,'只确认已勾选的孩子；未识别的保持原状态。','Confirm only checked students. Unknown faces leave the manifest unchanged.')}</p>}
 <footer className={s.tools}><PickupGalleryButton locale={locale} disabled={busy||saving} onImage={image=>{void analyze(image);}}/>{!shot?<button type="button" onClick={()=>void capture()} disabled={!ready} aria-label={text(locale,'拍照','Capture')}><Camera/></button>:<><button type="button" disabled={saving} onClick={retake} aria-label={text(locale,'重拍','Retake')}><RotateCcw/></button><button type="button" onClick={confirm} disabled={busy||saving} className={s.confirm}><Check/>{pickupIds.length?text(locale,`确认接到 ${pickupIds.length} 人`,`Confirm ${pickupIds.length} pickups`):text(locale,'完成','Done')}</button></>}</footer></dialog>;
}
