...
function CameraDialog({trip,locale,onUpdated,close,initialImage}:{trip:Trip;locale:Locale;onUpdated:(u:TripExecution)=>void;close:()=>void;initialImage:string}){
 ...
 <footer className={s.tools}><PickupGalleryButton locale={locale} disabled={busy||saving} onImage={image=>{void analyze(image);}}/>{!shot?<button type="button" onClick={()=>void capture()} disabled={!ready} aria-label={text(locale,'拍照','Capture')}><Camera/></button>:<><button type="button" disabled={saving} onClick={retake} aria-label={text(locale,'重拍','Retake')}><RotateCcw/></button><button type="button" onClick={confirm} disabled={busy||saving||!selected.some(Boolean)||selected.some(id=>id&&!eligible.some(r=>r.id===id))} aria-label={text(locale,'确认接到','Confirm pickup')}><Check/></button><span>{selected.filter(Boolean).length} {text(locale,'人待确认','selected')}</span></>}</footer></dialog>;
}
