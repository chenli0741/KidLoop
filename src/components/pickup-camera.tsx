...
function CameraDialog({trip,locale,onUpdated,close,initialImage}:{trip:Trip;locale:Locale;onUpdated:(u:TripExecution)=>void;close:()=>void;initialImage:string}){
 ...
 const [shot,setShot]=useState(initialImage),[ready,setReady]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(''),[progress,setProgress]=useState(''),[matches,setMatches]=useState<FaceMatch[]>([]),[selected,setSelected]=useState<string[]>([]);
