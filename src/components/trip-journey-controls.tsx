...
export function TripJourneyControls({ trip, locale, onUpdated }: { trip: Trip; locale: Locale; onUpdated: (update: TripExecution) => void }) {
  ...
  const label = finishing ? { zh: "结束行程", en: "Finish trip" } : inTransit ? { zh: `到达 · ${targetName}`, en: `Arrive · ${targetName}` } : dropoffRiders ? { zh: `送达 · ${targetName}`, en: `Drop off · ${targetName}` } : { zh: `出发 · ${targetName}`, en: `GO · ${targetName}` };
