...
export function PickupSettingForm({ kind, schoolId, locale, initial, rules=[], classrooms=[], programs=[], remove=false }: {
  kind: SettingKind; schoolId: string; locale: Locale; initial?: PickupSetting; rules?: PickupSetting[];
  classrooms?: {id:string;name:string}[]; programs?: {id:string;name:string}[]; remove?: boolean;
}) {
  return <SettingsForm action={saveSetting} submitLabel={remove ? text(locale,"确认删除","Confirm removal") : text(locale,"保存设置","Save settings")}>
    ...
    {remove ? <><input type="hidden" name="remove" value="1" /><p className="full">{text(locale,`删除“${initial?.name}”？对应规则的日程预览会重新计算。`,`Remove “${initial?.name}”? The schedule preview will be recalculated.`)}</p></> : <>
      ...
      {kind==="rule" && <>
        <fieldset className="pickup-checks full"><legend>{text(locale,"适用班级（可多选）","Classes (select one or more)")}</legend>{classrooms.map(c=><label key={c.id}><input type="checkbox" name="classroomIds" value={c.id} defaultChecked={initial?.classroomIds?.includes(c.id)} /><span>{c.name}</span></label>)}</fieldset>
