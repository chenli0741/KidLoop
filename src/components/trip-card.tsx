...
function TripSegmentContent({trip,journeyTrip,locale,interactive,onUpdated,showStops=true,cameraEnabled=false,showParentContact=true,role,selectedStopIndex,onSelectStop}:{trip:Trip;journeyTrip?:Trip;locale:Locale;interactive:boolean;onUpdated:(update:TripExecution)=>void;showStops?:boolean;cameraEnabled?:boolean;showParentContact?:boolean;role:"ADMIN"|"DRIVER";selectedStopIndex?:number;onSelectStop?:(index:number)=>void}) {
  ...
  return <>
      ...
      {showStops && (trip.routeStops?.length ? <ol className="fixed-trip-stops">{trip.routeStops.map((stop,i)=><li className={`${i === currentIndex ? "is-current " : ""}${i === (selectedStopIndex ?? currentIndex) ? "is-selected" : ""}`} key={stop.id}><div className="fixed-stop-select"><button type="button" className="fixed-stop-main" disabled={!onSelectStop} onClick={() => onSelectStop?.(i)}><span className="fixed-stop-number">{i+1}</span><span className="fixed-stop-copy"><span className="fixed-stop-title"><small>{stop.time}</small><strong>{stop.name}</strong></span><span className="fixed-stop-address">{stop.address}</span></span></button><LocationMap compact name={stop.name} address={stop.address}/></div></li>)}</ol> : <div className="route-strip">
