...
import {dropoffProgress,pickupProgress,rideManifest} from "@/lib/ride-manifest";
...
function TripSegmentContent({trip,journeyTrip,locale,interactive,onUpdated,showStops=true,cameraEnabled=false,showParentContact=true,role,selectedStopIndex,onSelectStop}:{trip:Trip;journeyTrip?:Trip;locale:Locale;interactive:boolean;onUpdated:(update:TripExecution)=>void;showStops?:boolean;cameraEnabled?:boolean;showParentContact?:boolean;role:"ADMIN"|"DRIVER";selectedStopIndex?:number;onSelectStop?:(index:number)=>void}) {
  ...
  const dropoff=dropoffProgress(visibleRiders);
  const showPickupProgress=manifest.mode==='stop'&&Boolean(currentStop?.schoolId);
  const showDropoffProgress=manifest.mode==='stop'&&Boolean(currentStop?.programId);
  const sharedPickupStop=showPickupProgress&&isSharedPickupStop(trip.riders,currentStop?.id);
  return <>
      ...
      <div className="manifest-header">
        ...
        <span>{manifest.mode==='completed'?text(locale,`共 ${visibleRiders.length} 名学生`,`${visibleRiders.length} students`):manifest.mode==='onboard'?text(locale,`车上 ${visibleRiders.length} 人 · ${trip.capacity} 座`,`${visibleRiders.length} on board · ${trip.capacity} seats`):sharedPickupStop?text(locale,`共享待确认 ${pickup.waiting} · 本车已接 ${pickup.pickedUp} · 应接 ${trip.sharedPickupMin ?? 0}–${trip.sharedPickupMax ?? trip.capacity} 人 · ${trip.capacity} 座`,`Shared unconfirmed ${pickup.waiting} · This vehicle picked up ${pickup.pickedUp} · target ${trip.sharedPickupMin ?? 0}–${trip.sharedPickupMax ?? trip.capacity} · ${trip.capacity} seats`):showPickupProgress?text(locale,`已接 ${pickup.pickedUp} · 待接 ${pickup.waiting} · ${trip.capacity} 座`,`Picked up ${pickup.pickedUp} · Waiting ${pickup.waiting} · ${trip.capacity} seats`):showDropoffProgress?text(locale,`已送达 ${dropoff.droppedOff} · 待送达 ${dropoff.waiting} · ${trip.capacity} 座`,`Dropped off ${dropoff.droppedOff} · Waiting ${dropoff.waiting} · ${trip.capacity} seats`):trip.hasSharedPickups ? text(locale,`共享接送：本车应接 ${trip.sharedPickupMin ?? 0}–${trip.sharedPickupMax ?? trip.capacity} 人 · ${trip.capacity} 座`,`Shared pickup: this vehicle should take ${trip.sharedPickupMin ?? 0}–${trip.sharedPickupMax ?? trip.capacity} riders · ${trip.capacity} seats`) : trip.routeName ? text(locale, `Pickup ${countedRiders.length} 人 · ${trip.capacity} 座`, `Pickup ${countedRiders.length} · ${trip.capacity} seats`) : text(locale, `Pickup ${countedRiders.length}/${trip.capacity} 个座位`, `Pickup ${countedRiders.length} of ${trip.capacity} seats`)}</span>
      </div>
      {sharedPickupStop&&<div className="shared-pickup-allocation" role="status">
        <strong>{text(locale,`共享候选共 ${trip.sharedPickupTotal ?? visibleRiders.length} 人 · 各车达到最低人数后可分别出发`,`Shared pool: ${trip.sharedPickupTotal ?? visibleRiders.length} riders · each vehicle may depart after reaching its minimum`)}</strong>
        <div>{trip.sharedPickupVehicles?.map(vehicle=><span key={vehicle.tripId} data-current={vehicle.tripId===trip.id||undefined}><b>{vehicle.tripId===trip.id?text(locale,"本车","This vehicle"):text(locale,"另一车","Other vehicle")} · {vehicle.vehicleName}</b>{text(locale,`至少 ${vehicle.min} 人 · 最多 ${vehicle.max} 人${vehicle.tripId===trip.id?` · 已接 ${pickup.pickedUp} 人`:''}`,`Minimum ${vehicle.min} · maximum ${vehicle.max}${vehicle.tripId===trip.id?` · picked up ${pickup.pickedUp}`:''}`)}</span>)}</div>
      </div>}
