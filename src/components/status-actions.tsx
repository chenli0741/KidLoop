...
export function StatusActions({ tripId, assignmentId, status, parentAbsent = false, atDropoff = false, onUpdated, targetTripId, role = "DRIVER", manifestPending = false, beginManifestUpdate, endManifestUpdate }: { tripId: string; atDropoff?: boolean; targetTripId?: string; assignmentId: string; status: RiderStatus; parentAbsent?: boolean; role?: "ADMIN" | "DRIVER"; onUpdated: (update: TripExecution) => void; manifestPending?: boolean; beginManifestUpdate?: () => boolean; endManifestUpdate?: () => void }) {
  ...
  async function update(next: RiderStatus) {
    if(saving.current||manifestPending||beginManifestUpdate?.()===false)return;
    saving.current=true;setPending(true);setError(null);
      try {
        const location = await captureOperationLocation();
        const result = await withOperationTimeout(updateRiderStatus(assignmentId, next, next === "EXCEPTION" || next === "ABSENT" ? { reason } : undefined, targetTripId, location));
      ...
      } finally {saving.current=false;setPending(false);endManifestUpdate?.();}
  }
    ...
    <div className="status-actions">
      <div className="status-button-row">
        ...
        {role === "DRIVER" && status === "SCHEDULED" ? (
          <button type="button" className="button compact primary" disabled={pending||manifestPending} onClick={() => update("PICKED_UP")}>
            <UserCheck size={16} /> {text(locale, "已接到", "Picked up")}
          </button>
        ) : null}
        {role === "DRIVER" && status === "PICKED_UP" && !atDropoff ? (
          <button type="button" className="icon-button" title={text(locale, "撤销接到，恢复待接送", "Undo pickup, return to scheduled")} aria-label={text(locale, "撤销接到", "Undo pickup")} disabled={pending||manifestPending} onClick={() => update("SCHEDULED")}>
            <RotateCcw size={17} />
          </button>
        ) : null}
        {role === "DRIVER" && status === "DROPPED_OFF" ? (
          <button type="button" className="icon-button" title={text(locale, "撤销送达，恢复已接到", "Undo drop-off, return to picked up")} aria-label={text(locale, "撤销送达", "Undo drop-off")} disabled={pending||manifestPending} onClick={() => update("PICKED_UP")}>
            <RotateCcw size={17} />
          </button>
        ) : null}
        {role === "ADMIN" && (status === "SCHEDULED" || status === "PICKED_UP") ? (
          <button type="button" className="icon-button" title={text(locale, "标记缺席", "Mark absent")} aria-label={text(locale, "标记缺席", "Mark absent")} disabled={pending||manifestPending} onClick={openReasonDialog}>
            <UserX size={17} />
          </button>
        ) : null}
        {role === "DRIVER" && status === "SCHEDULED" ? (
          <button type="button" className="icon-button danger" title={text(locale, "特殊原因", "Special reason")} aria-label={text(locale, "特殊原因", "Special reason")} disabled={pending||manifestPending} onClick={openReasonDialog}>
