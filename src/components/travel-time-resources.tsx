...
export async function TravelTimeResources() {
  ...
  return <section className="content-section resource-list-section"><div className="section-heading"><div><span className="eyebrow">{text(locale,"基础配置","Configuration")}</span><h2>{text(locale,"地点时间","Location times")}</h2><p className="form-hint">{text(locale,"记录 A 到 B 的行驶时间，以及从 A 点停留多久再出发。未记录的路段默认行驶 10 分钟。","Record travel time and origin dwell. Unrecorded journeys default to 10 minutes.")}</p></div><div className="resource-heading-actions"><span className="section-count">{rows.length}</span><ResourceDialog title={text(locale,"添加地点时间","Add location time")}><ActionForm action={createTravelTime} submitLabel={text(locale,"保存","Save")}>{fields(locale)}</ActionForm></ResourceDialog></div></div>
  ...
  <ExecutionHistory locale={locale}/></section>;
}
