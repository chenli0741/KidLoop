...
export function AccountForm({ drivers, students, initial, requestedAccount }: { requestedAccount?:{id:string;email:string}; initial?:EditableAccount; drivers: { id: string; name: string }[]; students: AccountStudent[] }) {
  const locale = useLocale();
  const [role, setRole] = useState<UserRole>(initial?.role ?? "PARENT");
  const Form = initial ? SettingsForm : ActionForm;
  return <Form action={initial ? updateAccount : createAccount} submitLabel={initial ? text(locale,"保存修改","Save changes") : text(locale, "批准并绑定账号", "Approve and bind account")}>
    {requestedAccount && <input type="hidden" name="requestId" value={requestedAccount.id}/>}
    {initial && <><input type="hidden" name="userId" value={initial.id}/><input type="hidden" name="updatedAt" value={initial.updatedAt}/></>}
    {initial && role !== "DRIVER" && <label><span>{text(locale, "姓名", "Name")}</span><input name="name" defaultValue={initial?.name} maxLength={100} required /></label>}
    <label><span>{text(locale, "登录邮箱", "Login email")}</span><input name="email" defaultValue={initial?.email ?? requestedAccount?.email} readOnly={!!initial || !!requestedAccount} type="email" maxLength={254} autoComplete="off" required /></label>
