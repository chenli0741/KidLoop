...
export function AccountForm({ drivers, students, initial }: { initial?:EditableAccount; drivers: { id: string; name: string }[]; students: { id: string; name: string; schoolName: string }[] }) {
  ...
  return <Form action={initial ? updateAccount : createAccount} submitLabel={initial ? text(locale,"保存修改","Save changes") : text(locale, "创建账号", "Create account")}>
    ...
    <label><span>{text(locale, "姓名", "Name")}</span><input name="name" defaultValue={initial?.name} maxLength={100} required /></label>
    ...
    {!initial && <label className="full"><span>{text(locale, "初始密码（12–128 字符）", "Initial password (12–128 characters)")}</span><input name="password" type="password" minLength={12} maxLength={128} autoComplete="new-password" required /></label>}
    {initial && <label className="full"><span>{text(locale,"电话","Phone")}</span><input name="phone" type="tel" maxLength={80} defaultValue={initial.phone}/></label>}
    ...
    {role === "PARENT" && <fieldset className="student-checklist"><legend>{initial ? text(locale,"绑定孩子","Linked children") : text(locale, "绑定孩子（至少一名）", "Link children (at least one)")}</legend>{students.map((student) => <label key={student.id}><input name="studentIds" type="checkbox" value={student.id} defaultChecked={initial?.student_ids.includes(student.id)} /><span><strong>{student.name}</strong><small>{student.schoolName}</small></span></label>)}</fieldset>}
  </Form>;
}
