...
test("school rules share calendars across routes without drivers and validate edits",async()=>{
  ...
  try{
    ...
    const classroom=await id("insert into classrooms(school_id,name) values($1,'B102')",[school]), foreignClass=await id("insert into classrooms(school_id,name) values($1,'Other')",[other]);
    ...
    const rule={schoolId:school,kind:"rule",name:"Regular",classroomIds:[classroom],weekdays:["1","2","4","5"],pickupTime:"14:30"};
    await assert.rejects(save({...rule,classroomIds:[foreignClass]}),/belonging/);
    await save(rule);await assert.rejects(save(rule),/already have/);
    ...
    let rows=await preview();assert.equal(rows.length,7);assert.equal(rows.filter(r=>r.date==="2026-09-07").length,0);
