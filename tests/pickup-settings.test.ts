...
test("school rules share calendars across routes without drivers and validate edits",async()=>{
  ...
  try{
    ...
    await c.query("insert into operating_terms(name,starts_on,ends_on) values('Term','2026-09-01','2026-12-31')");
    await c.query("insert into school_terms(school_id,name,starts_on,ends_on) values($1,'Fall','2026-09-07','2026-09-13')",[school]);
    await assert.rejects(save(term),/only be edited/);
